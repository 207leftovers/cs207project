...
def serialize(json_obj):
    ...
    output = json.dumps(json_obj).encode()

    #length = str(len(output)).zfill(LENGTH_FIELD_LENGTH)
    totallength = len(output)+LENGTH_FIELD_LENGTH
    length = (totallength ).to_bytes(LENGTH_FIELD_LENGTH, byteorder='little')
